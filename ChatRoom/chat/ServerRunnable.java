            ...
            while (true) {
                ...
                switch (msgType) {
                    ...
                    case ChatConstants.MSG_TYPE_CHAT:
                        ...
                        String tar = dis.readUTF();
                        String my = dis.readUTF();
                        //获取是谁发的消息   昵称
...
//                        System.out.println("昵称：" + userNickName + ":" + msgContent);
                        ...
                        break;
                    case ChatConstants.MSG_TYPE_MI:
                        String msgContent2 = dis.readUTF();
                        String targetName = dis.readUTF();
                        String clientName = dis.readUTF();
                        //获取是谁发的消息   昵称
                        String userNickName2 = ServerChat.onLineSocketMap.get(clientSocket);
                        //把收到的消息广播发给每个客户端socket
                        StringBuilder toSendMsgs = new StringBuilder();
                        LocalDateTime timeNow = LocalDateTime.now();
                        DateTimeFormatter datetf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
                        toSendMsgs.append(timeNow.format(datetf)).append("  ").append(userNickName2).append("对")
                                .append(targetName).append("说:")
                                .append("\r\n")
                                .append(msgContent2)
                                .append("\r\n")
                        ;
                        System.out.println("sendChatToAll:" + toSendMsgs);
                        sendChatToOne(targetName,clientName,toSendMsgs.toString());
                        break;
     ...
     * @param msg
     */
    private void sendChatToOne(String target,String myname ,String msg) {
        //key  socket    value: 昵称.
        ...
        ServerChat.onLineSocketMap.forEach((socket, nick) -> {
            try {
                ...
                //有人登陆，广播所有人的名字
                //首先刻一个3  代表  有人说话
                if(nick.contains(target) || nick.contains(myname)){
