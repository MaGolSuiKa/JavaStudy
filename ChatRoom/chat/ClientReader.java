            ...
            while (true) {
                ...
                switch (msgType) {
                    ...
                    case  ChatConstants.MSG_TYPE_MI:
                        String msgsContent = dis.readUTF();
                        String targetName = dis.readUTF();
                        System.out.println("有人发言：" + msgsContent);
                        //if(ClientChat.clientName.equals(targetName)){
                            //界面：把消息追加到界面的聊天记录上
                            clientChat.smsContent.append(msgsContent);
                            //滾動到底端 ，始终显示最新的消息.
                            clientChat.smsContent.setCaretPosition(clientChat.smsContent.getText().length());
                        //}
                }
